          ...
          <div className="relative flex flex-col items-center space-y-4">
            ...
            {t.levels.map((level: any, index: number) => (
              <React.Fragment key={level.id}>
                <div className={`w-full max-w-lg bg-slate-900 border ${index === 2 ? 'border-orange-500 shadow-[0_0_30px_rgba(234,88,12,0.15)]' : 'border-white/10'} rounded-xl p-5 relative group hover:border-white/30 transition-colors`}>
                  <div className="flex items-start gap-4">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${index === 2 ? 'bg-orange-500/20 text-orange-500' : 'bg-slate-800 text-slate-400'}`}>
                      <span className="font-bold text-lg">{level.id}</span>
                    </div>
                    <div>
                      <h3 className={`text-lg font-bold uppercase tracking-tight mb-2 ${index === 2 ? 'text-orange-400' : 'text-white'}`}>
                        {level.title}
                      </h3>
                      ...
                      {level.sub && (
                        <div className="p-3 bg-slate-950/50 rounded-lg border border-white/5 text-xs text-slate-400 leading-relaxed whitespace-pre-line">
                          {level.sub}
                        </div>
                      )}
                    </div>
